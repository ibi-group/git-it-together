...
module.exports = function server () {
  ...
  const slackBotDisplayName = env.SLACK_BOT_DISPLAY_NAME || 'git-it-together'
  const slackBotUserName = env.SLACK_BOT_USER_NAME || 'git-it-together'
  let botId
  ...
  bot.on('close', () => {
    // while using the slackbot, the bot seemed to eventually close or lose its
    // websocket connection and wouldn't automatically reconnect.  If a close
    // event is fired, try to reconnect.
    bot = connectToSlack()
  })
